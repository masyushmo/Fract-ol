...
t_core		*create_win(char *stt)
{
	...
	if (!(core = ft_memalloc(sizeof(t_core))))
		stop("ERROR: some problem with mlx, my bad :(");
	if(!(core->mlx_ptr = mlx_init()) || \
		(!(core->win_ptr = mlx_new_window(core->mlx_ptr, W_X, W_Y, stt))) || \
		(!(core->image = create_image(core))))
		{
			destroy_win(core);
			stop("ERROR: some problem with mlx, my bad :(");
		}
